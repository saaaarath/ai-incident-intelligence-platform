...
- Incident Management: Automatically converts threshold-crossing anomalies into `Incident` entities (`id`, `title`, `severity`, `status`, `primaryService`, `startedAt`, `detectedAt`, `resolvedAt`) with lifecycle management (`OPEN`, `INVESTIGATING`, `RESOLVED`, `CLOSED`), dynamic severity upgrading, and duplicate incident prevention during active failure windows.
